...
# AppData folder
appdata = Path(os.getenv("APPDATA")) / "BSSStickerManager"
appdata.mkdir(exist_ok=True)

db_path = appdata / "stickerdb.db"

# First run only: copy template DB
if not db_path.exists():
    template_db = resource_path("stickerdb_template.db")
...
while True:
    ...
    if cmd=='help':
        ...
        imported=str(input("Input the data exported before (TYPE 'cancel' TO CANCEL):"))
        if imported.lower()!="cancel":
            imports(imported)
    elif cmd=="inv":
